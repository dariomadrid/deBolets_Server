...
var Cacera = new Schema({
	idCacera: { type: Number, unique: true },
...
var CaceraModel = mongoose.model('Cacera', Cacera);
...
app.get('/api', function (req, res) {
  res.send('API debolets.cat is running on port 4242.');
});

// Create cacera
app.post('/api/caceres', function (req, res) {
  var cacera;
  console.log("POST: ");
  console.log(req.body);
  cacera = new CaceraModel({
  ...
  cacera.save(function (err) {
    if (!err) {
      return console.log("cacera created");
    } else {
...
//Llista caceres
app.get('/api/caceres', function (req, res) {
  return CaceraModel.find(function (err, caceres) {
    if (!err) {
      return res.send(caceres);
    } else {
      return console.log(err);
    }
  });
...
// Single product
app.get('/api/caceres/:id', function (req, res) {
  return CaceraModel.find(req.params.id, function (err, cacera) {
    if (!err) {
      return res.send(cacera);
    } else {
      return console.log(err);
